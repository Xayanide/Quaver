...
export default {
	...
	async execute(interaction) {
		...
		if (player.queue.current.isStream) return interaction.replyHandler.reply(`**[${escapeMarkdown(player.queue.current.title)}](${player.queue.current.uri})**\n🔴 **${getGuildLocale(interaction.guildId, 'MISC.LIVE')}** ${'▬'.repeat(10)}${player.paused ? ' ⏸️' : ''}${player.queue.loop.type !== LoopType.None ? ` ${player.queue.loop.type === LoopType.Queue ? '🔁' : '🔂'}` : ''}${player.bassboost ? ' 🅱️' : ''}\n\`[${getGuildLocale(interaction.guildId, 'MISC.STREAMING')}]\` | ${getGuildLocale(interaction.guildId, 'MISC.ADDED_BY', player.queue.current.requester)}`, { ephemeral: true });
		return interaction.replyHandler.reply(`**[${escapeMarkdown(player.queue.current.title)}](${player.queue.current.uri})**\n${bar}${player.paused ? ' ⏸️' : ''}${player.queue.loop.type !== LoopType.None ? ` ${player.queue.loop.type === LoopType.Queue ? '🔁' : '🔂'}` : ''}${player.bassboost ? ' 🅱️' : ''}${player.nightcore ? ' 🇳' : ''}\n\`[${elapsedString} / ${durationString}]\` | ${getGuildLocale(interaction.guildId, 'MISC.ADDED_BY', player.queue.current.requester)}`, { ephemeral: true });
	},
};
