...
export default class DataHandler {
	...
	constructor(opts) {
		...
		this.localGuilds = new Collection();
		this.schema = {
			settings: {
				stay: {
					enabled: false,
	...
	async get(key, item) {
		/** @type {Values} */
		const localData = this.localGuilds.get(key);
		if (localData) return _get(localData, item);
		/** @type {Values} */
		const data = await this.cache.get(key);
		if (!data) await this.cache.set(key, this.schema);
		this.localGuilds.set(key, data);
	...
	async set(key, item, value) {
		...
		if (!data) data = {};
		this.localGuilds.set(key, _set(data, item, value));
