...
export default class PlayerHandler {
	...
	async disconnect(channelId) {
		...
		if (voiceChannel.stageInstance?.topic !== getGuildLocale(this.player.guildId, 'MISC.STAGE_TOPIC')) return;
		try {
	...
	async locale(code, { args = [], type = 'neutral', components = null, files = null } = {}) {
		return this.send(getGuildLocale(this.player.guildId, code, ...args), { type, components, files });
	}
}
