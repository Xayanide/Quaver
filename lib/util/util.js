...
import { cachedDatabase, locales } from './common.js';
 ...
 */
export function getGuildLocale(guildId, string, ...vars) {
	const cdb = cachedDatabase.get(guildId);
	const guildLocale = cdb.settings.locale;
	return getLocale(guildLocale ?? defaultLocale, string, ...vars);
}
...
export async function settingsPage(interaction, guildLocale, option) {
	...
	switch (option) {
		...
		case 'format': {
			...
			current = cdb.settings.format ?? 'simple';
			actionRow.addComponents(
				new ButtonBuilder()
					.setCustomId('format_simple')
					.setLabel(getGuildLocale(interaction.guildId, 'CMD.SETTINGS.MISC.FORMAT.OPTIONS.SIMPLE'))
					.setStyle(current === 'simple' ? ButtonStyle.Success : ButtonStyle.Secondary)
					.setDisabled(current === 'simple'),
				new ButtonBuilder()
					.setCustomId('format_detailed')
					.setLabel(getGuildLocale(interaction.guildId, 'CMD.SETTINGS.MISC.FORMAT.OPTIONS.DETAILED'))
					.setStyle(current === 'detailed' ? ButtonStyle.Success : ButtonStyle.Secondary)
					.setDisabled(current === 'detailed'),
			);
			embeds = current === 'simple' ? [
				new EmbedBuilder()
					.setDescription(`${getGuildLocale(interaction.guildId, 'MUSIC.PLAYER.PLAYING.NOW.SIMPLE', escapeMarkdown(getGuildLocale(interaction.guildId, 'CMD.SETTINGS.MISC.FORMAT.EXAMPLE.SIMPLE')), 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', '4:20')}\n${getGuildLocale(interaction.guildId, 'MISC.ADDED_BY', interaction.user.id)}`)
					.setColor(colors.neutral),
			] : [
				new EmbedBuilder()
					.setTitle(getGuildLocale(interaction.guildId, 'MUSIC.PLAYER.PLAYING.NOW.DETAILED.TITLE'))
					.setDescription(`**[${escapeMarkdown(getGuildLocale(interaction.guildId, 'CMD.SETTINGS.MISC.FORMAT.EXAMPLE.DETAILED'))}](https://www.youtube.com/watch?v=dQw4w9WgXcQ)**`)
					.addFields([
						{ name: getGuildLocale(interaction.guildId, 'MUSIC.PLAYER.PLAYING.NOW.DETAILED.DURATION'), value: '`4:20`', inline: true },
						{ name: getGuildLocale(interaction.guildId, 'MUSIC.PLAYER.PLAYING.NOW.DETAILED.UPLOADER'), value: 'Rick Astley', inline: true },
						{ name: getGuildLocale(interaction.guildId, 'MUSIC.PLAYER.PLAYING.NOW.DETAILED.ADDED_BY'), value: `<@${interaction.user.id}>`, inline: true },
					])
					.setThumbnail('https://i.ytimg.com/vi/dQw4w9WgXcQ/hqdefault.jpg'),
			];
			current = getGuildLocale(interaction.guildId, `CMD.SETTINGS.MISC.FORMAT.OPTIONS.${current.toUpperCase()}`);
		}
	}
