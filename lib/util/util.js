 ...
 * @returns {string|import('discord.js').MessageOptions|import('discord.js').MessagePayload|import('discord.js').InteractionReplyOptions|import('discord.js').WebhookEditMessageOptions} The MessageOptions object.
 */
export function buildMessageOptions(msgData, { type = 'neutral', components = null, files = null } = {}) {
	...
	msgData = msgData.map(el => {
		if (typeof el === 'string') return new EmbedBuilder().setDescription(el).setColor(colors[type]);
		if (!el.data.color) return el.setColor(colors[type]);
	});
	console.log(msgData);
